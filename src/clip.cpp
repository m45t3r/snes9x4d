		...
		for (register int w = 5; w >= 0; w--) {
			...
			if (w == 5) // The colour window...
			{
				if (c == 0) // ... on the main screen
				{
					if ((Memory.FillRAM[0x2130] & 0xc0) ==
						...
						continue;
					} else if ((Memory.FillRAM[0x2130] &
						    0xc0) == 0x00)
						continue;
				} else {
					// .. colour window on the sub-screen.
					if ((Memory.FillRAM[0x2130] & 0x30) ==
					    0x30) {
						// The sub-screen is switched
						// off, completely clip
						// everything.
						for (i = 0; i < 6; i++) {
							ippu->Clip[1].Count[i] =
			...
			if (!Settings.DisableGraphicWindows) {
				if (w == 5 || pClip->Count[5] ||
					...
					if (w == 5 ||
					    (Memory.FillRAM[0x212c + c] &
					     Memory.FillRAM[0x212e + c] &
					     (1 << w))) {
						if (ppu->ClipWindow1Enable[w]) {
							if (!ppu->ClipWindow1Inside
								 [w]) {
								Win1[Window1Enabled]
								    .Left =
								    ppu->Window1Left;
					...
					if (Window1Enabled && Window2Enabled) {
						// Overlap logic
						//
						// Each window will be in one of
						// three states:
						// 1. <no range> (Left > Right.
						// One band)
						// 2. |    ---------------- |
						// (Left >= 0, Right <= 255,
						// Left <= Right. One band)
						// 3. |------------ ----------|
						// (Left1 == 0, Right1 < Left2;
						// Left2 > Right1, Right2 ==
						// 255. Two bands)
						...
						if (invert) {
							int b;
							int j = 0;
							int empty_band_count =
							    0;

							// First remove all
							// empty bands from the
							// list.
							for (b = 0; b < B;
							     b++) {
								if (!BAND_EMPTY(
						...
						// Only one window enabled so no
						// need to perform complex
						// overlap logic...
