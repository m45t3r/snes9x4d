...
#define BANDS_INTERSECT(A, B) ((A.Left >= B.Left && A.Left < B.Right) || (A.Right > B.Left && A.Right <= B.Right))
...
void ComputeClipWindows()
{
	...
	for (int c = 0; c < 2; c++, pClip++) {
		// Loop around the colour window then a clip window for each of
			...
			if (!Settings.DisableGraphicWindows) {
				if (w == 5 || pClip->Count[5] ||
						...
						if (ppu->ClipWindow1Enable[w]) {
							if (!ppu->ClipWindow1Inside[w]) {
								...
								if (ppu->Window1Left <= ppu->Window1Right) {
									...
									// 'outside' a window with no range - appears to
									// be the whole screen.
									Win1[Window1Enabled].Left = 0;
									Win1[Window1Enabled++].Right = 256;
								}
					...
					if (Window1Enabled && Window2Enabled) {
						...
						// 2. | ---------------- | (Left >= 0, Right <= 255, Left <= Right.One
						// band)
						// 3. | ---------------------- | (Left1 == 0, Right1<Left2; Left2>
						// Right1, Right2 == 255. Two bands)
						...
						case CLIP_OR:
							if (Window1Enabled == 1) {
								...
								if (BAND_EMPTY(Win2[0])) {
									// Window 2 defines an empty range - just use
									// window 1 as the clipping (which could also be
									// empty).
									B = Window1Enabled;
									memmove(Bands, Win1,
						...
						if (invert) {
							...
							if (j > 0) {
								...
								// Inverting a window that consisted of only empty bands
								// is the whole width of the screen. Needed for Mario
								// Kart's rear-view mirror display.
								if (empty_band_count) {
									pClip->Left[j][w] = 0;
									pClip->Right[j][w] = 256;
									j++;
						...
						// Only one window enabled so no need to perform complex overlap
						// logic...
					...
					if (w != 5) {
						if (pClip->Count[5]) {
							// Colour window enabled. Set the clip windows for all remaining
							// backgrounds to be the same as the colour window.
							if (pClip->Count[w] == 0) {
								...
								// Intersect the colour window with the bg's own clip
								// window.
								pClip->Count[w] = pClip->Count[5];
								for (uint32 i = 0; i < pClip->Count[w]; i++) {
									if (pClip->Left[i][5] >= pClip->Right[i][5]) {
										...
										pClip->Left[i][w] = pClip->Left[i][5];
										pClip->Right[i][w] = pClip->Right[i][5];
									} else if (pClip->Left[i][w] <=
										   pClip->Right[i][w]) {
										// Intersect the bands of the two
										// windows.
										if (pClip->Count[5] ==
										    pClip->Count[w]) {
											pClip->Left[i][w] =
