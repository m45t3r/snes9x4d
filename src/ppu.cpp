...
void S9xSetPPU(uint8 Byte, uint16 Address, struct SPPU *ppu, struct InternalPPU *ippu)
{
	if (Address <= 0x2183) {
		...
		case 0x2103:
			...
			// Only update the sprite write address top bit if the
			// low byte has been written to first.
			if (ppu->OAMFlip & 2) {
				ppu->OAMAddr &= 0x00FF;
		...
		case 0x2105:
			// Screen mode (0 - 7), background tile sizes and
			// background 3 priority
		...
		case 0x211a:
			// Mode 7 outside rotation area display mode and
			// flipping
		...
		case 0x212a:
			...
			break;
		case 0x212b:
			// Windows 1 & 2 overlap logic for objects and colour
			// window
			...
			break;
		case 0x212c:
			// Main screen designation (backgrounds 1 - 4 and
			// objects)
			...
			break;
		case 0x212d:
			// Sub-screen designation (backgrounds 1 - 4 and
			// objects)
		...
		case 0x2131:
			...
			if (Byte != Memory.FillRAM[0x2131]) {
				...
				// Backgrounds 1 - 4, objects and backdrop
				// colour add/sub enable
#ifdef DEBUGGER
				if (Byte & 0x80) {
					// Subtract
		...
		case 0x2137:
			// Software latch for horizontal and vertical timers
			// (read-only)
			return;
		case 0x2138:
		...
		case 0x213c:
		case 0x213d:
			// Horizontal and vertical (low/high) read counter
			// (read-only)
			return;
		case 0x213e:
...
void S9xSetCPU(uint8 byte, uint16 Address, struct SPPU *ppu, struct SCPUState *cpu)
{
	...
	if (Address < 0x4200) {
		...
		default:
			...
			if (Settings.TraceUnknownRegisters) {
				sprintf(String,
					"Unknown register register write: "
					"$%02X->$%04X\n",
					byte, Address);
				S9xMessage(S9X_TRACE, S9X_PPU_TRACE, String);
			}
		...
		case 0x4200:
			...
			if (byte & 0x10) {
				...
				// No need to check for HTimer being disabled as
				// the scanline event trigger code won't trigger
				// an H-IRQ unless its enabled.
...
uint8 S9xGetCPU(uint16 Address, struct InternalPPU *ippu, CMemory *mem)
{
	...
	if (Address < 0x4200) {
		...
		switch (Address) {
		// BS Dynami Tracer! needs to be able to check if NMIs are
		// enabled already, otherwise the game locks up.
		case 0x4200:
		...
		case 0x420b:
			...
			// requires this register should not always return zero.
			// .. But Aero 2 waits until this register goes zero..
			// Just keep toggling the value for now in the hope that
			// it breaks the game out of its wait loop...
...
void S9xSuperFXExec()
{
	...
	if (tmp == -1) // 0x018428) //0x01bfc3) //0x09edaf) //-1) //0x57edaf)
	{
		while (Memory.FillRAM[0x3030] & 0x20) {
			...
			/* Check if any registers changed (and print them if
			 * they did) */
			for (i = 0; i < 16; i++) {
				uint32 a = 0;
