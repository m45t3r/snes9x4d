...
//                printf("22 Trapezoid!\n");
                ...
                for(int line=0; line<2; line++)
				{
                    ...
                    if(C4CosTable[angle]==0) break;
                    int32 tmp = (int32)(((int64)C4SinTable[angle])<<16)/C4CosTable[angle];
                    int32 cnt = (int16)(READ_WORD(Memory.C4RAM+0x1f83) - READ_WORD(Memory.C4RAM+0x1f89) - 1);
                    for(int j=0; j<225; j++)
					{
                        int16 res;
                        if(cnt>=0)
						{
							...
							{
                                res = 0;
                                if(line) Memory.C4RAM[j+0x800]=1;
                            }
							else if(res>=255)
							{
                                res = 255;
                                if(line && Memory.C4RAM[j+0x800]==255)
								{ // XXX: Or does the real MMX3 have the bug this fixes?
                                    res = 254;
                                }
                            }
                        }
