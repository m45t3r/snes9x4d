...
extern int ZEXPORT unzOpenCurrentFile(unzFile file)
{
	...
	case UNZ_DEFLATED:
		 ...
		 * "dummy" byte after the compressed stream in order to complete
		 * decompression and return Z_STREAM_END. In unzip, i don't wait
		 * absolutely Z_STREAM_END because I known the size of both
		...
		case UNZ_DEFLATED:
			if ((pfile_in_zip_read_info->stream.avail_in == 0) &&
				...
				if (fseek(
					...
					SEEK_SET) != 0)
					return UNZ_ERRNO;
				if (fread(pfile_in_zip_read_info->read_buffer,
					  uReadThis, 1,
...
extern int ZEXPORT unzGetLocalExtrafield(unzFile file, voidp buf, unsigned len)
{
	...
	if (fread(buf, (uInt)size_to_read, 1, pfile_in_zip_read_info->file) !=
	    1)
		return UNZ_ERRNO;
