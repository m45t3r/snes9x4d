...
void DSP2_Op05()
{
	...
	// The max size bitmap is limited to 255 because the size parameter is a
	// byte I think size=0 is an error.  The behavior of the chip on size=0
	// is to return the last value written to DR if you read DR on Op05 with
	// size = 0.  I don't think it's worth implementing this quirk unless
...
#if 0 // Probably no reason to use this code - it's not quite bit accurate and
      // it doesn't look as good as Overload's algorithm
