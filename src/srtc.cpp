...
void S9xUpdateSrtcTime()
{
	...
	// on the system clock and adding the same number of seconds to the
	// S-RTC clock structure. I originally tried using mktime and localtime
	// library functions to keep track of time but some of the GNU time
	// functions fail when the year goes to 2099 (and maybe less) and this
	// would have caused a bug with DKJM2 so I'm doing it this way to get
	// around that problem.

	// Note: Dai Kaijyu Monogatari II only allows dates in the range
	// 1996-21xx.
...
uint8 S9xGetSRTC(uint16 Address)
{
	if (rtc.mode == MODE_READ) {
		if (rtc.index < 0) {
			S9xUpdateSrtcTime(); // Only update it if the game reads
					     // it
			rtc.index++;
