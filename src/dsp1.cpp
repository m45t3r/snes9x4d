...
void DSP1SetByte(uint8 byte, uint16 address)
{
	if ((address & 0xf000) == 0x6000 || (address & 0x7fff) < 0x4000) {
		//		if ((address & 1) == 0)
		//		{
		if ((DSP1.command == 0x0A || DSP1.command == 0x1A) &&
		    DSP1.out_count != 0) {
			DSP1.out_count--;
			DSP1.out_index++;
			return;
		...
		if (DSP1.waiting4command ||
			...
			DSP1.first_parameter = FALSE;
			   ...
			   (DSP1.in_count != 0 ||
			    (DSP1.in_count == 0 && DSP1.in_index == 0))) {
		}
		//		else if (DSP1.first_parameter)
		//		{
		//		}
			...
			if (DSP1.in_count) {
				...
				if (--DSP1.in_count == 0) {
					...
					case 0x1f:
						...
						break;
					case 0x00: // Multiple
						Op00Multiplicand = (int16)(
						    DSP1.parameters[0] |
						    (DSP1.parameters[1] << 8));
						Op00Multiplier = (int16)(
						    DSP1.parameters[2] |
						    (DSP1.parameters[3] << 8));
					...
					case 0x20: // Multiple
						Op20Multiplicand = (int16)(
						    DSP1.parameters[0] |
						    (DSP1.parameters[1] << 8));
						Op20Multiplier = (int16)(
						    DSP1.parameters[2] |
						    (DSP1.parameters[3] << 8));
					...
					case 0x1c: // Polar (3D rotate)
						Op1CZ =
						    (DSP1.parameters[0] |
						     (DSP1.parameters[1] << 8));
						// MK: reversed X and Y on
						// neviksti and John's advice.
						Op1CY =
						    (DSP1.parameters[2] |
						     (DSP1.parameters[3] << 8));
						Op1CX =
					...
					case 0x02: // Parameter (Projection)
						Op02FX =
						    (short)(DSP1.parameters[0] |
							    (DSP1.parameters[1]
							     << 8));
						Op02FY =
						    (short)(DSP1.parameters[2] |
							    (DSP1.parameters[3]
							     << 8));
						Op02FZ =
						    (short)(DSP1.parameters[4] |
							    (DSP1.parameters[5]
							     << 8));
						Op02LFE =
						    (short)(DSP1.parameters[6] |
							    (DSP1.parameters[7]
							     << 8));
						Op02LES =
						    (short)(DSP1.parameters[8] |
							    (DSP1.parameters[9]
							     << 8));
						Op02AAS =
						    (unsigned short)(DSP1.parameters
									 [10] |
								     (DSP1.parameters
									  [11]
								      << 8));
			...
			if (--DSP1.out_count == 0) {
				...
				if (DSP1.command == 0x1f) {
					if ((DSP1.out_index % 2) != 0) {
						t = (uint8)
						    DSP1ROM[DSP1.out_index >>
							    1];
					} else {
						t = DSP1ROM[DSP1.out_index >>
							    1] >>
						    8;
					}
				}
	...
	if ((address & 0xf000) == 0x6000 ||
		...
		if (DSP1.in_count == DSP1.in_index) {
			...
			case 0x01:
				...
				break;
			case 0x09:
				// Multiply - don't yet know if this is signed
				// or unsigned
				DSP2Op09Word1 = DSP1.parameters[0] |
						(DSP1.parameters[1] << 8);
				DSP2Op09Word2 = DSP1.parameters[2] |
						(DSP1.parameters[3] << 8);
				DSP1.out_count = 4;
				WRITE_DWORD(DSP1.output,
					    DSP2Op09Word1 * DSP2Op09Word2);
				break;
			case 0x05:
				if (DSP2Op05HasLen) {
	...
	if ((address & 0xf000) == 0x6000 ||
		...
		if (!DSP4.waiting4command && DSP4.in_count == DSP4.in_index) {
			...
			case 0x0000: {
				...
				DSP4_Multiply(multiplicand, multiplier,
					      product);
...
#undef PRINT
			...
			case 0x000A: {
				...
				// NOTE: Snes9x only!
				// For some odd reason, the input nybbles are
				// reversed
				...
				// Hack: Reverse the outputs for now to
				// compensate
