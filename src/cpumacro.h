			...
			struct SICPU *icpu, struct SCPUState *cpu)
{
	...
	if (CHECKDECIMAL()) {
		...
		uint8 Ans8 = (A2 << 4) | A1;
		if (~(reg->AL ^ Work8) & (Work8 ^ Ans8) & 0x80)
			SETOVERFLOW();
		else
			CLEAROVERFLOW();
		reg->AL = Ans8;
		...
		if (~(reg->AL ^ Work8) & (Work8 ^ (uint8)Ans16) & 0x80)
			SETOVERFLOW();
		else
			CLEAROVERFLOW();
		reg->AL = (uint8)Ans16;
			 ...
			 struct SICPU *icpu, struct SCPUState *cpu)
{
	...
	if (CHECKDECIMAL()) {
		...
		if (~(reg->A.W ^ Work16) & (Work16 ^ Ans16) & 0x8000)
			SETOVERFLOW();
		else
			CLEAROVERFLOW();
		reg->A.W = Ans16;
		...
		if (~(reg->A.W ^ Work16) & (Work16 ^ (uint16)Ans32) & 0x8000)
			SETOVERFLOW();
		else
			CLEAROVERFLOW();
			 ...
			 struct SICPU *icpu, struct SCPUState *cpu)
{
	...
	if (CHECKDECIMAL()) {
		...
		uint16 Ans16 = (A4 << 12) | (A3 << 8) | (A2 << 4) | (A1);
		if ((reg->A.W ^ Work16) & (reg->A.W ^ Ans16) & 0x8000)
			SETOVERFLOW();
		else
			CLEAROVERFLOW();
		reg->A.W = Ans16;
		...
		if ((reg->A.W ^ Work16) & (reg->A.W ^ (uint16)Int32) & 0x8000)
			SETOVERFLOW();
		else
			CLEAROVERFLOW();
			...
			struct SICPU *icpu, struct SCPUState *cpu)
{
	...
	if (CHECKDECIMAL()) {
		...
		uint8 Ans8 = (A2 << 4) | A1;
		if ((reg->AL ^ Work8) & (reg->AL ^ Ans8) & 0x80)
			SETOVERFLOW();
		else
			CLEAROVERFLOW();
		reg->AL = Ans8;
