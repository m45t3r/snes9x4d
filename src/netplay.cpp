...
bool8 S9xNPConnectToServer(const char *hostname, int port, const char *rom_name)
{
	if (!S9xNPInitialise())
		return (FALSE);
	...
	if (GUI.ClientSemaphore == NULL)
		GUI.ClientSemaphore =
		    CreateSemaphore(NULL, 0, NP_JOYPAD_HIST_SIZE, NULL);
...
up correctly?");
	...
	if (connect(NetPlay.Socket, (struct sockaddr *)&address,
		    sizeof(address)) < 0) {
		char buf[100];
#ifdef __WIN32__
...
on the remote machine on this port?");
			...
			sprintf(
			    buf,
			    "Connection to server failed with error number %d",
#ifdef __WIN32__
			    WSAGetLastError()
#else
			    errno
	...
	*ptr++ = NP_CLNT_HELLO;
	...
	if (!S9xNPGetData(NetPlay.Socket, header, 7) ||
		...
		S9xNPSetError(
		    "Error in 'HELLO' reply packet received from server.");
		S9xNPDisconnect();
		return (FALSE);
	}
#ifdef NP_DEBUG
	...
	if (S9xNPConnect()) {
		...
		while (NetPlay.Connected) {
			if (S9xNPWaitForHeartBeat()) {
				LONG prev;
				if (!ReleaseSemaphore(GUI.ClientSemaphore, 1,
						      &prev)) {
#ifdef NP_DEBUG
					printf(
					    "CLIENT: ReleaseSemaphore failed - "
					    "already hit max count (%d) %ld\n",
					    NP_JOYPAD_HIST_SIZE,
					    S9xGetMilliTime() - START);
#endif
					S9xNPSetWarning(
					    "NetPlay: Client may be out of "
					    "sync with server.");
				} else {
...
bool8 S9xNPWaitForHeartBeat()
{
	...
	while (S9xNPGetData(NetPlay.Socket, header, 3 + 4)) {
		if (header[0] != NP_SERV_MAGIC) {
			S9xNPSetError("Bad magic value from server while "
				      "waiting for heart-beat message\n");
			S9xNPDisconnect();
			return (FALSE);
		}
		if (header[1] != NetPlay.ServerSequenceNum) {
			char buf[200];
			sprintf(buf,
				"Unexpected message sequence number from "
				"server, expected %d, got %d\n",
		...
		if ((header[2] & 0x1f) == NP_SERV_JOYPAD) {
			...
			if (num) {
				if (!S9xNPGetData(NetPlay.Socket,
						  header + 3 + 4, num * 4)) {
					S9xNPSetError("Error while receiving "
						      "'JOYPAD' message.");
					S9xNPDisconnect();
					return (FALSE);
				}
			}
			NetPlay.Frame[NetPlay.JoypadWriteInd] =
			    READ_LONG(&header[3]);
			...
			for (i = 0; i < num; i++)
				NetPlay.Joypads[NetPlay.JoypadWriteInd][i] =
				    READ_LONG(
					&header[3 + 4 + i * sizeof(uint32)]);

			for (i = 0; i < NP_MAX_CLIENTS; i++)
				NetPlay
				    .JoypadsReady[NetPlay.JoypadWriteInd][i] =
				    TRUE;
			...
			if (NetPlay.JoypadWriteInd !=
				...
				// printf ("(%d)", (NetPlay.JoypadWriteInd -
				// NetPlay.JoypadReadInd) %
			...
			case NP_SERV_PAUSE:
				...
				if (NetPlay.Paused)
					S9xNPSetWarning(
					    "CLIENT: Server has paused.");
				else
					S9xNPSetWarning(
					    "CLIENT: Server has resumed.");
				break;
...
bool8 S9xNPLoadROM(uint32 len)
{
	...
	if (!S9xNPLoadROMDialog((char *)data)) {
		S9xNPSetError("Disconnected from NetPlay server because you "
...
bool8 S9xNPGetROMImage(uint32 len)
{
	...
	if (CalculatedSize + 5 >= len ||
		...
		S9xNPSetError(
		    "Size error in ROM image data received from server.");
		S9xNPDisconnect();
		return (FALSE);
	}
	...
	if (filename_len > PATH_MAX ||
	    !S9xNPGetData(NetPlay.Socket, (uint8 *)Memory.ROMFilename,
			  filename_len)) {
		S9xNPSetError(
		    "Error while receiving ROM filename from server.");
		S9xNPDisconnect();
		Settings.StopEmulation = TRUE;
		return (FALSE);
	}
...
void S9xNPGetFreezeFile(uint32 len)
{
	...
	if (!S9xNPGetData(NetPlay.Socket, frame_count, 4)) {
		S9xNPSetError("Error while receiving freeze file information "
			      "from server.");
		S9xNPDisconnect();
		return;
	}
	...
	if (tmpnam(fname)) {
...
#endif
		{
			if (fwrite(data, 1, len, file) == len) {
				fclose(file);
#ifndef __WIN32__
				/* We need .s96 extension, else .s96 is addded
				 * by unix code */
				char buf[PATH_MAX + 1];
...
#endif
					S9xNPSetError("Unable to load freeze "
						      "file just received.");
			} else {
				S9xNPSetError("Failed to write to temporary "
					      "freeze file.");
				fclose(file);
			}
		} else
...
void S9xNPStepJoypadHistory()
{
	if ((NetPlay.JoypadReadInd + 1) % NP_JOYPAD_HIST_SIZE !=
		...
		NetPlay.JoypadReadInd =
		    (NetPlay.JoypadReadInd + 1) % NP_JOYPAD_HIST_SIZE;
		if (NetPlay.FrameCount !=
		    NetPlay.Frame[NetPlay.JoypadReadInd]) {
			S9xNPSetWarning("This Snes9X session may be out of "
					"sync with the server.");
#ifdef NP_DEBUG
			printf("*** CLIENT: client out of sync with server "
			       "(%d, %d) @%ld\n",
			       NetPlay.FrameCount,
			       NetPlay.Frame[NetPlay.JoypadReadInd],
			       S9xGetMilliTime() - START);
...
void S9xNPResetJoypadReadPos()
{
	...
	for (int h = 0; h < NP_JOYPAD_HIST_SIZE; h++)
		memset((void *)&NetPlay.Joypads[h], 0,
		       sizeof(NetPlay.Joypads[0]));
	for (int h = 0; h < NP_JOYPAD_HIST_SIZE; h++)
...
bool8 S9xNPSendData(int socket, const uint8 *data, int length)
{
	...
	do {
		...
		// Write the data in small chunks, allowing this thread to spot
		// an abort request from another thread.
		if (num_bytes > 512)
			num_bytes = 512;
...
#ifdef NP_DEBUG
				printf("CLIENT: EINTR, EAGAIN or EWOULDBLOCK "
				       "while sending data @%ld\n",
				       S9xGetMilliTime() - START);
#endif
				continue;
			}
...
bool8 S9xNPGetData(int socket, uint8 *data, int length)
{
	...
	do {
		...
		// Read the data in small chunks, allowing this thread to spot
		// an abort request from another thread.
		if (num_bytes > chunk)
			num_bytes = chunk;
		...
		if (got < 0) {
			...
			if (errno != WSAEMSGSIZE)
				...
				got = num_bytes;
#ifdef NP_DEBUG
				printf("CLIENT: WSAEMSGSIZE, actual bytes %d "
				       "while receiving data @%ld\n",
...
bool8 S9xNPInitialise()
{
	...
	if (!initialised) {
		...
		if (WSAStartup(MAKEWORD(1, 0), &data) != 0) {
			S9xNPSetError("Call to init Windows sockets failed. Do "
				      "you have WinSock2 installed?");
			return (FALSE);
		}
	}
#endif
...
{
	// Discard any pending heartbeats and wait for any frame that is
	// currently being emulated to complete.
#ifdef NP_DEBUG
