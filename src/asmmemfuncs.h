...
#define memset32(_dst, _c, _count)                                                                                     \
	({                                                                                                             \
		uint32_t *dst = (_dst);                                                                                \
		uint32_t c __asm__("r7") = (_c);                                                                       \
		int count = (_count);                                                                                  \
		uint32_t dummy0 __asm__("r4"), dummy1 __asm__("r5"), dummy2 __asm__("r6");                             \
		__asm__ __volatile__("      cmp   %[count], #4\n"                                                      \
				     "      blt   2f\n"                                                                \
				     "      mov   %[dummy0], %[c]\n"                                                   \
				     "      tst   %[dst], #4\n"                                                        \
				     "      strne %[c], [%[dst]], #4\n"                                                \
				     ...
				     "      strge %[c], [%[dst]], #4\n"                                                \
				     "\n"                                                                              \
				     : [ dst ] "+&r"(dst), [ count ] "+&r"(count), [ dummy0 ] "=&r"(dummy0),           \
				       [ dummy1 ] "=&r"(dummy1), [ dummy2 ] "=&r"(dummy2), [ c ] "+&r"(c)              \
				     :                                                                                 \
				     : "cc", "memory");                                                                \
		_dst;                                                                                                  \
	})
...
#define memset16(_dst, _c, _count)                                                                                     \
	({                                                                                                             \
		uint16_t *dst = (_dst);                                                                                \
		uint16_t c __asm__("r7") = (_c);                                                                       \
		int count = (_count);                                                                                  \
		uint32_t dummy0 __asm__("r4"), dummy1 __asm__("r5"), dummy2 __asm__("r6");                             \
		__asm__ __volatile__("      cmp   %[count], #2\n"                                                      \
				     "      blt   3f\n" /* Alignment is known to be at least 16-bit */                 \
				     "      tst   %[dst], #2\n"                                                        \
				     "      strneh %[c], [%[dst]], #2\n"                                               \
				     "      subne  %[count], %[count], #1\n" /* Now we are 32-bit aligned (need to     \
										upgrade 'c' to 32-bit )*/              \
				     "      orr    %[c], %[c], %[c], asl #16\n"                                        \
				     "      mov   %[dummy0], %[c]\n"                                                   \
				     "      cmp   %[count], #8\n"                                                      \
