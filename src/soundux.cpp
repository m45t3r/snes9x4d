...
void S9xSetEnvRate(Channel *ch, unsigned long rate, int direction, int target)
{
	...
	static int steps[] = {//	0, 64, 1238, 1238, 256, 1, 64, 109, 64, 1238
			      0, 64, 619, 619, 128, 1, 64, 55, 64, 619};

	if (rate == 0 || so.playback_rate == 0)
		ch->erate = 0;
	else {
		ch->erate = (unsigned long)(((int64)FIXED_POINT * 1000 * steps[ch->state]) / (rate * so.playback_rate));
	}
}
...
void S9xSetFilterCoefficient(int tap, int value)
{
	...
	SoundData.no_filter = (FilterTaps[0] == 127 || FilterTaps[0] == 0) && FilterTaps[1] == 0 &&
			      FilterTaps[2] == 0 && FilterTaps[3] == 0 && FilterTaps[4] == 0 && FilterTaps[5] == 0 &&
			      FilterTaps[6] == 0 && FilterTaps[7] == 0;
}
		     ...
		     int release_rate)
{
	...
	case SOUND_DECAY:
		...
		// S9xSetEnvelopeRate(channel, decay_rate, -1, (MAX_ENVELOPE_HEIGHT * (sustain_level + 1)) >> 3);
		break;
	case SOUND_SUSTAIN:
...
void DecodeBlock(Channel *ch)
{
	...
	// different waveform.
	if (!Settings.DisableSampleCaching && memcmp((uint8 *)compressed, &IAPU.ShadowRAM[ch->block_pointer], 9) == 0) {
		...
		if (!Settings.DisableSampleCaching) {
			memcpy(IAPU.CachedSamples + (ch->block_pointer << 2), (uint8 *)ch->decoded, 32);
		}
	}
	ch->block_pointer += 9;
}
...
void MixStereo(int sample_count)
{
	...
	for (uint32 J = 0; J < NUM_CHANNELS; J++) {
		...
		// uncommented by jonathan gevaryahu, as it is necessary for most cards in linux
...
void S9xMixSamples(uint8 *buffer, int sample_count) { S9xMixSamplesO(buffer, sample_count, 0); }
...
#endif
	{
		...
		if (so.mute_sound) {
			...
			if (sd->echo_enable && sd->echo_buffer_size) {
				if (so.stereo) {
					...
					if (sd->no_filter) {
						...
						for (J = 0; J < sample_count; J++) {
							...
							Echo[sd->echo_ptr] =
							    (E * sd->echo_feedback) / 128 + EchoBuffer[J];

							if ((sd->echo_ptr += 1) >= sd->echo_buffer_size)
								sd->echo_ptr = 0;

							I = (MixBuffer[J] * sd->master_volume[J & 1] +
							     E * sd->echo_volume[J & 1]) /
							    VOL_DIV16;
				...
				for (J = 0; J < sample_count; J++) {
					I = (MixBuffer[J] * sd->master_volume[J & 1]) / VOL_DIV16;
...
#endif
		{
			if (sd->echo_enable && sd->echo_buffer_size) {
				...
				for (J = 0; J < sample_count; J++) {
					I = (MixBuffer[J] * sd->master_volume[J & 1]) / VOL_DIV8;
					CLIP8(I);
					buffer[J + O] = I + 128;
				}
			}
