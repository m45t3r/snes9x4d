...
{
	uint32 addr =
	    (((APU.DSP[APU_DIR] << 8) + (sample_number << 2)) & 0xffff);
...
void S9xSetEnvRate(Channel *ch, unsigned long rate, int direction, int target)
{
	...
	if (rate == 0 || so.playback_rate == 0)
		...
		ch->erate = (unsigned long)(((int64)FIXED_POINT * 1000 *
					     steps[ch->state]) /
...
void S9xSetMasterVolume(short volume_left, short volume_right)
{
	if (Settings.DisableMasterVolume) {
		...
		if (!so.stereo)
			volume_left =
			    (ABS(volume_right) + ABS(volume_left)) / 2;
		...
		SoundData.master_volume[1 ^ Settings.ReverseStereo] =
		    volume_right;
	}
}
...
void S9xFixSoundAfterSnapshotLoad()
{
	...
	for (int i = 0; i < 8; i++) {
		...
		SoundData.channels[i].envxx = SoundData.channels[i].envx
					      << ENVX_SHIFT;
		SoundData.channels[i].next_sample = 0;
		SoundData.channels[i].interpolate = 0;
		SoundData.channels[i].previous[0] =
		    (int32)SoundData.channels[i].previous16[0];
		SoundData.channels[i].previous[1] =
		    (int32)SoundData.channels[i].previous16[1];
	}
	SoundData.master_volume[Settings.ReverseStereo] =
	    SoundData.master_volume_left;
	SoundData.master_volume[1 ^ Settings.ReverseStereo] =
	    SoundData.master_volume_right;
	SoundData.echo_volume[Settings.ReverseStereo] =
	    SoundData.echo_volume_left;
	SoundData.echo_volume[1 ^ Settings.ReverseStereo] =
...
void S9xSetFilterCoefficient(int tap, int value)
{
	...
	SoundData.no_filter = (FilterTaps[0] == 127 || FilterTaps[0] == 0) &&
			      FilterTaps[1] == 0 && FilterTaps[2] == 0 &&
			      FilterTaps[3] == 0 && FilterTaps[4] == 0 &&
			      FilterTaps[5] == 0 && FilterTaps[6] == 0 &&
			      FilterTaps[7] == 0;
}
		     ...
		     int sustain_rate, int sustain_level, int release_rate)
{
	...
	case SOUND_ATTACK:
		S9xSetEnvRate(&SoundData.channels[channel], attack_rate, 1,
			      127);
		//		S9xSetEnvelopeRate (channel, attack_rate, 1,
		// 127);
		break;
	...
	case SOUND_DECAY:
		...
		//			(MAX_ENVELOPE_HEIGHT * (sustain_level +
		// 1))
		//>> 3);
		break;
	case SOUND_SUSTAIN:
		S9xSetEnvRate(&SoundData.channels[channel], sustain_rate, -1,
			      0);
		//		S9xSetEnvelopeRate (channel, sustain_rate, -1,
		// 0);
		break;
	}
}
...
void S9xSetEnvelopeHeight(int channel, int level)
{
	...
	if (ch->envx == 0 && ch->state != SOUND_SILENT &&
	    ch->state != SOUND_GAIN) {
...
void S9xSetSoundFrequency(int channel, int hertz)
{
	if (so.playback_rate) {
		...
		SoundData.channels[channel].frequency =
		    (int)(((int64)hertz * FIXED_POINT) / so.playback_rate);
		if (Settings.FixFrequency) {
			SoundData.channels[channel].frequency =
			    (unsigned long)((double)SoundData.channels[channel]
						.frequency *
					    0.980);
		}
	}
...
//	else
//		DecodeBlockAsm2 (compressed, raw, &ch->previous [0],
//&ch->previous [1]);
#else
	...
	case 1:
		for (i = 8; i != 0; i--) {
			...
			prev0 = (int16)prev0;
			*raw++ = prev1 =
			    ((int32)sample1 << shift) + prev0 - (prev0 >> 4);
			prev1 = (int16)prev1;
			*raw++ = prev0 =
			    ((int32)sample2 << shift) + prev1 - (prev1 >> 4);
		}
		break;
	case 2:
		for (i = 8; i != 0; i--) {
...
void AltDecodeBlock2(Channel *ch)
{
	...
	case 0:
		for (i = 8; i != 0; i--) {
			...
			sample2 >>= 4;
			// Sample 1 = Top Nibble, shifted down and Sign
			// Extended.
			sample1 >>= 4;
	...
	case 2:
		for (i = 8; i != 0; i--) {
			...
			out = ((sample1 << shift) * 256 + (prev0 & ~0x2) * 488 -
			       prev1 * 240) >>
			      8;
			...
			*raw++ = (int16)out;
			...
			out = ((sample2 << shift) * 256 + (prev0 & ~0x2) * 488 -
			       prev1 * 240) >>
			      8;
...
void DecodeBlock(Channel *ch)
{
	...
	// re-using part of the original sample but generate a slightly
	// different waveform.
	if (!Settings.DisableSampleCaching &&
	    memcmp((uint8 *)compressed, &IAPU.ShadowRAM[ch->block_pointer],
		   9) == 0) {
		ch->block = (signed short *)(IAPU.CachedSamples +
					     (ch->block_pointer << 2));
		ch->previous[0] = ch->block[15];
		ch->previous[1] = ch->block[14];
	} else {
		if (!Settings.DisableSampleCaching)
			memcpy(&IAPU.ShadowRAM[ch->block_pointer],
			       (uint8 *)compressed, 9);
		compressed++;
		signed short *raw = ch->block = ch->decoded;
		...
		// all bits of the data for that block except for the sign bit
		// of each one
		if ((filter & 0xF0) < 0xD0) {
...
void MixStereo(int sample_count)
{
	...
	for (uint32 J = 0; J < NUM_CHANNELS; J++) {
		...
		if (ch->needs_decode) {
			...
			if (Settings.InterpolatedSound && freq0 < FIXED_POINT &&
			    !mod)
				ch->interpolate =
		...
		for (uint32 I = 0; I < (uint32)sample_count; I += 2) {
			...
			if (ch->env_error >= FIXED_POINT) {
				uint32 step =
				    ch->env_error >> FIXED_POINT_SHIFT;
				...
				ch->left_vol_level =
				    (ch->envx * ch->volume_left) / 128;
				ch->right_vol_level =
				    (ch->envx * ch->volume_right) / 128;
				VL = (ch->sample * ch->left_vol_level) / 128;
				VR = (ch->sample * ch->right_vol_level) / 128;
			}
			...
			if (ch->count >= FIXED_POINT) {
				...
				if (ch->interpolate) {
					int32 s =
					    (int32)ch->sample + ch->interpolate;

					CLIP16(s);
					ch->sample = (int16)s;
					VL = (ch->sample * ch->left_vol_level) /
					     128;
					VR =
					    (ch->sample * ch->right_vol_level) /
					    128;
				}
			}
			...
			ch->echo_buf_ptr[I + (1 ^ Settings.ReverseStereo)] +=
			    VR;
		}
	stereo_exit:;
	}
...
void S9xMixSamplesO(uint8 *buffer, int sample_count, int byte_offset)
{
	...
	if (!so.mute_sound) {
		...
		if (sd->echo_enable)
			memset(EchoBuffer, 0,
			       sample_count * sizeof(EchoBuffer[0]));
		if (so.stereo)
...
#endif
	{
		...
		if (so.mute_sound) {
		    ...
		    if (so.encoded) {
			for (J = 0; J < sample_count; J++) {
				I = (MixBuffer[J] * sd->master_volume_left) /
				    VOL_DIV16;
				CLIP16(I);
...
void S9xSetPlaybackRate(uint32 playback_rate)
{
	...
	so.err_rate = (uint32)(SNES_SCANLINE_TIME * FIXED_POINT /
			       (1.0 / (double)so.playback_rate));
	S9xSetEchoDelay(APU.DSP[APU_EDL] & 0xf);
	for (int i = 0; i < 8; i++)
		S9xSetSoundFrequency(i, SoundData.channels[i].hertz);
}
...
void S9xPlaySample(int channel, struct SAPU *apu)
{
	...
	S9xFixEnvelope(channel, apu->DSP[APU_GAIN + (channel << 4)],
	...
	case MODE_ADSR:
		if (ch->attack_rate == 0) {
			...
			S9xSetEnvRate(ch, ch->attack_rate, 1,
				      MAX_ENVELOPE_HEIGHT);
		}
		ch->envxx = ch->envx << ENVX_SHIFT;
		break;
