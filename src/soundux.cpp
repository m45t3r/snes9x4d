...
int32 env_counter_table[32] = {0x0000, 0x000F, 0x0014, 0x0018, 0x001E, 0x0028, 0x0030, 0x003C, 0x0050, 0x0060, 0x0078,
			       0x00A0, 0x00C0, 0x00F0, 0x0140, 0x0180, 0x01E0, 0x0280, 0x0300, 0x03C0, 0x0500, 0x0600,
			       0x0780, 0x0A00, 0x0C00, 0x0F00, 0x1400, 0x1800, 0x1E00, 0x2800, 0x3C00, 0x7800};
...
static uint32 OldAttackRate[16] = {4100, 2600, 1500, 1000, 640, 380, 260, 160, 96, 64, 40, 24, 16, 10, 6, 1};

static uint32 OldDecayRate[8] = {1200, 740, 440, 290, 180, 110, 74, 37};

static uint32 OldSustainRate[32] = {UINT_MAX, 38000, 28000, 24000, 19000, 14000, 12000, 9400, 7100, 5900, 4700,
				    3500,     2900,  2400,  1800,  1500,  1200,	 880,	740,  590,  440,  370,
				    290,      220,   180,   150,   110,	  92,	 74,	55,   37,   18};

static int OldNoiseFreq[32] = {0,    16,   21,	 25,   31,   42,   50,	 63,	84,    100,  125,
...
static inline int32 absolute(int32 x) { return ((x < 0) ? -x : x); }

static inline int16 sclip15(int32 x) { return (int16)(((x & 16384) ? (x | ~16383) : (x & 16383))); }

static inline int8 sclamp8(int32 x) { return (int8)(((x > 127) ? 127 : (x < -128) ? -128 : x)); }

static inline int16 sclamp15(int32 x) { return (int16)(((x > 16383) ? 16383 : (x < -16384) ? -16384 : x)); }

static inline int16 sclamp16(int32 x) { return (int16)(((x > 32767) ? 32767 : (x < -32768) ? -32768 : x)); }

static inline uint8 *sample_address(uint8 srcn) { return (IAPU.RAM + ((APU.DSP[APU_DIR] << 8) + (srcn << 2))); }
...
void S9xSetEchoFeedback(int feedback) { SoundData.echo_feedback = feedback; }
	...
	if (!restore) {
		for (i = 0; i < NUM_CHANNELS; i++) {
			...
			if (env_counter_max < ch->xenv_count)
				...
				ch->env_error = (uint32)((double)FIXED_POINT / env_counter_max *
							 (env_counter_max - ch->xenv_count));

			if (ch->xenv_rate < 0)
				ch->erate = 0;
			else
				ch->erate = (uint32)((double)FIXED_POINT / env_counter_max * ch->xenv_rate);
...
static void S9xConvertSoundOldValues()
{
	...
	for (i = 0; i < NUM_CHANNELS; i++) {
		...
		ch->xenv_count = env_counter_max - (int32)((double)ch->env_error * env_counter_max / FIXED_POINT);
...
void S9xSetFilterCoefficient(int tap, int value)
{
	...
	SoundData.no_filter = FilterTaps[0] == 127 && FilterTaps[1] == 0 && FilterTaps[2] == 0 && FilterTaps[3] == 0 &&
			      FilterTaps[4] == 0 && FilterTaps[5] == 0 && FilterTaps[6] == 0 && FilterTaps[7] == 0;
}
...
static void S9xSetSoundFrequency(int channel, int hertz)
{
	if (so.playback_rate)
		SoundData.channels[channel].frequency =
		    (uint32)((int64)(hertz << (FIXED_POINT_SHIFT - 15)) * 32000 / so.playback_rate);
...
void MixStereo(int sample_count)
{
	...
	if (APU.DSP[APU_NON]) {
		...
		for (uint32 I = 0; I < (uint32)sample_count; I += 2) {
			...
			while (noise_count <= 0) {
				SoundData.noise_seed =
				    (SoundData.noise_seed >> 1) |
				    (((SoundData.noise_seed << 14) ^ (SoundData.noise_seed << 13)) & 0x4000);
				...
				if (ch->sample_pointer == SOUND_DECODE_LENGTH) {
					...
					if (ch->last_block) {
						...
						if (!ch->loop) {
							...
							ch->sample_number =
							    APU.DSP[APU_SRCN +
								    J * 0x10]; // SRCN might be different than before,
									       // so update it. This apparently fixes
									       // Mystical Ninja pipe sound.
...
void S9xResetSound(bool8 full)
{
	for (int i = 0; i < NUM_CHANNELS; i++) {
		...
		if (full) {
			...
			for (int j = 0; j < 2; j++)
				SoundData.channels[i].previous[j] = 0;
			for (int j = 0; j < 2; j++)
				SoundData.channels[i].previous16[j] = 0;
			for (int j = 0; j < 16; j++)
				SoundData.channels[i].decoded[j] = 0;
			for (int j = 0; j < 4; j++)
				SoundData.channels[i].nb_sample[j] = 0;
			SoundData.channels[i].nb_index = 0;
		}
	}
