...
struct FxRegs_s {
	...
	int x[32];
	uint32 vScreenHeight;	  /* 128, 160, 192 or 256 (could be overriden by
				     cmode) */
	...
	uint8 avCacheBackup[512]; /* Backup of ROM when the cache has replaced
				     it */
	uint32 vCounter;
	uint32 vInstCount;
	uint32 vSCBRDirty; /* if SCBR is written, our cached screen pointers
			      need updating */
};
