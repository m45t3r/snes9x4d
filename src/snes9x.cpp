		...
		if (*argv[i] == '-') {
			if (strcasecmp(argv[i], "-so") == 0 ||
				...
				Settings.ForceNoTransparency = FALSE;
				   ...
				   strcasecmp(argv[i], "-notransparency") ==
				       0) {
			...
			} else if (strcasecmp(argv[i], "-displayframerate") ==
				       0 ||
				...
				if (i + 1 < argc) {
					...
					uint8 byte;
					const char *error;
					if ((error = S9xGameGenieToRaw(
						 argv[++i], address, byte)) ==
					    NULL)
						S9xAddCheat(TRUE, FALSE,
							    address, byte);
					else
						S9xMessage(
						    S9X_ERROR,
				...
				if (i + 1 < argc) {
					...
					uint8 byte;
					const char *error;
					if ((error = S9xProActionReplayToRaw(
						 argv[++i], address, byte)) ==
					    NULL)
						S9xAddCheat(TRUE, FALSE,
							    address, byte);
			...
			} else if (strcasecmp(argv[i], "-nosamplecaching") ==
				       0 ||
				   strcasecmp(argv[i], "-nsc") == 0 ||
				   strcasecmp(argv[i], "-nc") == 0) {
				Settings.DisableSampleCaching = TRUE;
			} else if (strcasecmp(argv[i], "-nomastervolume") ==
				       0 ||
				...
				Settings.SoundEnvelopeHeightReading = TRUE;
				Settings.InterpolatedSound = TRUE;
			} else if (strcasecmp(argv[i], "-interpolatedsound") ==
				       0 ||
