...
bool8 SetupSound (long rate, bool8 sixteen_bit, bool8 stereo)
{
    ...
    if (Settings.SoundDriver >= WIN_FMOD_DIRECT_SOUND_DRIVER)
    {
        ...
        if (DirectX.SetSoundMode ())
        {
            ...
            SoundBuffer += 32 * 1024;
            ZeroMemory (SoundBuffer, _blocksize * _buffernos);
...
void S9xSyncSpeed( void)
{
    ...
    if (!Settings.TurboMode && Settings.SkipFrames == AUTO_FRAMERATE)
    {
        ...
        if ((success = ReleaseSemaphore (GUI.FrameTimerSemaphore, 1, &prev)) &&
            prev == 0)
        {
            // Mop up the ReleaseSemaphore above...
            WaitForSingleObject (GUI.FrameTimerSemaphore, 0);
            // ...and then wait for the real sync signal
            WaitForSingleObject (GUI.FrameTimerSemaphore, 100);
            IPPU.RenderThisFrame = TRUE;
            IPPU.SkippedFrames = 0;
        }
        else
    ...
    if (!VOODOO_MODE && !OPENGL_MODE)
    {
        ...
        if (PrimarySurfaceLockFailed)
        {
            ...
            if (GUI.Stretch)
            {
                ...
                dstRect.right = (dstRect.right - dstRect.left) + p.x;
                dstRect.bottom = (dstRect.bottom - dstRect.top) + p.y;
                dstRect.top = p.y;
                dstRect.left = p.x;
            }
            else
