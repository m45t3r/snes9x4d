	...
	do {
		...
		if (got < 0) {
			...
			if (errno != WSAEMSGSIZE)
				...
				got = num_bytes;
#ifdef NP_DEBUG
				printf("SERVER: WSAEMSGSIZE, actual bytes %d "
				       "while receiving data @%d\n",
...
#ifdef NP_DEBUG
				printf("SERVER: EINTR, EAGAIN or EWOULDBLOCK "
...
void S9xNPProcessClient(int c)
{
	...
	case NP_CLNT_HELLO:
		...
		if (!S9xNPSGetData(NPServer.Clients[c].Socket, data, len - 7)) {
			S9xNPSetWarning("SERVER: Failed to get HELLO message "
					"content from client.");
			S9xNPShutdownClient(c, TRUE);
			return;
		}
	...
	case NP_CLNT_READY:
		...
		// NPServer.Clients [c].SendSequenceNum, S9xGetMilliTime () -
		// START);
		if (NPServer.NumClients > NP_ONE_CLIENT) {
...
void S9xNPAcceptClient(int Listen, bool8 block)
{
	...
	if (i >= NP_MAX_CLIENTS) {
		S9xNPSetError("SERVER: Maximum number of NetPlay Clients have "
			      "already connected.");
		close(new_fd);
		return;
	}
	...
	if (remote_address.sin_family == AF_INET) {
		...
		if (host) {
#ifdef NP_DEBUG
			printf("SERVER: resolved new client's hostname (%s) "
			       "@%ld\n",
...
#ifdef NP_DEBUG
			printf("SERVER: couldn't resolve new client's hostname "
			       "(%s) @%ld\n",
...
void S9xNPSendROMLoadRequest(const char *filename)
{
	...
	*ptr++ = NP_SERV_LOAD_ROM;
	...
	for (int i = NP_ONE_CLIENT; i < NP_MAX_CLIENTS; i++) {
		if (NPServer.Clients[i].SaidHello) {
#ifdef NP_DEBUG
			printf("SERVER: Sending load ROM requesting to player "
			       "%d @%ld\n",
