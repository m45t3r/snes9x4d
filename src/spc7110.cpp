 ...
 *						and John Weidman, with fixes and
 *debugging code from CaitSith2
 *
...
uint8 S9xGetSPC7110(uint16 Address)
{
	...
	switch (Address) {
		...
		// AlignBy is set (afaik) at decompression time, and is the
		// offset multiplier bank50internal is an internal pointer to
		// the actual byte to read. so you read from offset*multiplier +
		// bank50internal the offset registers cannot be incremented due
		// to the offset multiplier.
	case 0x4800: {
	...
	case 0x4808:
		return s7r.reg4808;
	// C-Length low
	// counts down the number of bytes left to read from the decompression
	// buffer. this is set by the ROM, and wraps on bounds.
	case 0x4809:
		return s7r.reg4809;
	// C Length high
	case 0x480A:
	...
	case 0x4840:
		if (!Settings.SPC7110RTC)
			return Address >> 8;
		return s7r.reg4840;
		// command/index/value of RTC (essentially, zero unless we're in
		// read mode
	case 0x4841:
		if (!Settings.SPC7110RTC)
			return Address >> 8;
...
void S9xSetSPC7110(uint8 data, uint16 Address)
{
	...
	// C r/w? I have no idea. If you get weird values written here before a
	// bug, The Dumper should probably be contacted about running a test.
	case 0x4808:
		s7r.reg4808 = data;
		break;
...
void S9xUpdateRTC()
{
	...
	// Keep track of game time by computing the number of seconds that pass
